...
/*   Updated: 2026/09/05 23:19:05 by nribakov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_node	*get_next(t_ctx *c, t_node *current_pipeline_node)
{
	if ((!(pipeline_node->flags & FLAG_AND_IF)
		...
		return (get_next_after_or(c,
				pipeline_node)) else return (get_next_after_and(c,
				pipeline_node));
}
